...
export class CompanyService {
  ...
  udateCompany(company: CompanyI): Observable<UserResponseI | void> {
    return this.http.patch<UserResponseI | void>(`${environment.baseUrl}/company/${company.id}`, company).pipe(
      map((res: UserResponseI) => {
        return res;
      })
    );
